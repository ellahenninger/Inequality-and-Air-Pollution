...
# Load and match data revision: HDI
...
## Load the data CO2
#-------------------------------------------------------------------------------
hdi <- read.csv2("data/Controls/hdi.csv", 
                 header = T, 
                 sep = ";", 
                 dec = ".",
                 stringsAsFactors = F)
...
## Common country identifier
colnames(hdi)[2:9] <- c(1990, 2000, 2010, 2015, 2018:2021)
...
hdi_long$year <- as.numeric(as.character(hdi_long$year))

# join to prior data
data <- left_join(data, hdi_long, by = c("country", "year"))
