...
The following file replicates all analyses and plots to be found in the paper "Beyond the Haze: Decomposing the Effect of Economic Inequality on Global Air Quality from 2000-2020".
...
color_ci <- ifelse(coefs_main$CI_low > 0 & coefs_main$CI_up < 0|
                   coefs_main$CI_low < 0 & coefs_main$CI_up > 0,
                   adjustcolor("darkgrey", alpha = 0.7), viridis(3, alpha = 0.4)[1]) 
...
png("Plots/Coefficients_subgroups.png", width = 480, height = 400, units = "px", pointsize = 15)
...
## Robustness checks - Pollutant-related

```{r robustness pollutant}
...
# m4_re_wb_no2_f <- lm(lead_log_no2_combo ~ demeaned_inequ + 
#                       demeaned_income + demeaned_income_sq + demeaned_winning +
#                       demeaned_trade + demeaned_cpi + demeaned_industrial + demeaned_pop +
#                       year + as.factor(country),
#                      data = data_descript)
# 
# ## Let's see how many outliers we have, and what they are
# outliers <- rownames(m4_re_wb_no2_f$model)[rstandard(m4_re_wb_no2_f) > 1.5]
# outliers_detail <- data_descript[rownames(data_descript) %in% outliers,]
# data_no_outliers <- data_descript[!(rownames(data_descript) %in% outliers), ]
# 
# plot(fitted.values(m4_re_wb_no2),
#      residuals(m4_re_wb_no2))
...
## Robustness checks - HDI instead of GDP per capita

```{r hdi}

## data for this analysis
data_hdi <- data[which(data$year >= 2018),]

means_hdi <- aggregate(data_hdi$hdi,
                       by = list(data_hdi$country),
                       FUN = mean, na.rm = T)

means_hdi[which(means_hdi$x == "NaN"),] <- NA

## rename
names(means_hdi) <- c("country", "mean")

## put them back into the data_hdi frame
data_hdi$mean_hdi <- means_hdi$mean[match(data_hdi$country, means_hdi$country)]

## calculate de-meaned variable:
data_hdi$demeaned_hdi <- data_hdi$hdi - data_hdi$mean_hdi


m4_re_wb_hdi <- lmer(lead_log_PM25 ~ mean_inequ + demeaned_inequ + 
                       demeaned_hdi + demeaned_winning +
                       demeaned_trade + demeaned_cpi + demeaned_industrial + demeaned_pop +
                       mean_hdi + mean_winning +
                       mean_trade + mean_cpi + mean_industrial + mean_pop + year + 
                      (1 | country),
                     data = data_hdi)

stargazer(m4_re_wb_hdi, type = "text", omit = "country")
...
ggplot() +
geom_map(data = world, map = world,
         aes(long, lat, map_id = region, fill = trend_ineq)) +
...
# set colors
color_est <- ifelse(marginal_effects_mean_income_s$lower > 0 & marginal_effects_mean_income_s$upper < 0|
                    marginal_effects_mean_income_s$lower < 0 & marginal_effects_mean_income_s$upper > 0,
                    "darkgrey", viridis(3)[1])

color_ci <- ifelse(marginal_effects_mean_income_s$lower > 0 & marginal_effects_mean_income_s$upper < 0|
                   marginal_effects_mean_income_s$lower < 0 & marginal_effects_mean_income_s$upper > 0,
                   adjustcolor("darkgrey", alpha = 0.4), viridis(3, alpha = 0.4)[1]) 
...
color_ci <- ifelse(res_top10$CI_low > 0 & res_top10$CI_up < 0|
                   res_top10$CI_low < 0 & res_top10$CI_up > 0,
                   adjustcolor("darkgrey", alpha = 0.55), viridis(3, alpha = 0.4)[1]) 
